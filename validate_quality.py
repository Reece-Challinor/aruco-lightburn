...
def test_single_marker_quality():
    ...
    assert 0 in unique and 255 in unique, "Marker should have pure black (0) and white (255)"
    ...
    for row in range(1, marker.shape[0] - 1):
        if not np.array_equal(marker[row], marker[row-1]) and \
           not np.array_equal(marker[row], marker[row+1]):
