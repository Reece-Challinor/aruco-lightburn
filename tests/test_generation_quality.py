...
class TestGenerationQuality:
    ...
    def test_no_line_artifacts_in_single_marker(self):
        ...
        marker = self.generator.generate_marker(
            marker_id=0, dict_name="4X4_50", size_pixels=200
        )
        ...
        ), "Marker should have pure black and white"
    ...
    def test_scaling_preserves_quality(self):
        ...
        for size in sizes:
            ...
            assert marker.shape == (
                size,
                size,
            ), f"Marker should be {size}x{size} pixels"
    ...
    def test_grid_generation_alignment(self):
        ...
        # Generate a 3x3 grid
        markers = self.generator.generate_grid(
            start_id=0, dict_name="4X4_50", rows=3, cols=3, size_mm=25, spacing_mm=5
        )
    ...
    def test_fallback_pattern_quality(self):
        ...
        pattern = self.generator._create_fallback_pattern(
            marker_id, dict_name, size_pixels
        )
        ...
        assert pattern.shape == (
            size_pixels,
            size_pixels,
        ), "Pattern should match requested size"
    ...
    def _check_no_thin_lines(self, image, max_thin_line_width=1):
        ...
        for row in range(1, image.shape[0] - 1):
            ...
            if not np.array_equal(image[row], image[row - 1]) and not np.array_equal(
