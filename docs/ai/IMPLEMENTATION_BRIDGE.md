...
> - Per AGENTS.md: also update `docs/ai/walkthrough.md` (Summary/Changes/Tests/
>   Commit Plan format) and `docs/ai/task.md` for every completed prompt.
...
> **CI (Phase 0 finding C-3):** this is the first JS-test prompt — add
> `actions/setup-node` to `.github/workflows/ci.yml`, a `make test-js` target
> (`node --test static/js/**/*.test.js` or equivalent), and wire it into
> `make test`.
...
> **CSP (Phase 0 finding C-2):** update `app.py:272` script-src to
> `'self' 'wasm-unsafe-eval'` (narrowest wasm-permitting directive; do NOT use
> 'unsafe-eval'); no worker-src change needed (same-origin worker files pass
> script-src 'self'). Add a security-header test asserting the new directive.
...
> **Permissions-Policy (Phase 0 finding C-1):** update `app.py:297` to
> `camera=(self), microphone=(), geolocation=()` — the current `camera=()`
> globally blocks getUserMedia. Update `tests/test_security.py` header
