...
class DetectionValidator:
    ...
    def _place_markers_on_canvas(self, canvas, aruco_dict, scales, marker_ids, canvas_size_mm):
        ...
        for scale, marker_id in zip(scales, marker_ids):
            ...
            if (y_px + marker_size_px <= canvas.shape[0] and 
                x_px + marker_size_px <= canvas.shape[1]):
    ...
    def _create_pattern_metadata(self, dictionary, scales, canvas_size_mm, canvas_shape, test_markers):
    ...
    def _analyze_detected_marker(self, report, detected_id, expected_id, corners, image):
        ...
        if detected_id == expected_id:
            ...
            report['errors'].append(f"ID mismatch: expected {expected_id}, got {detected_id}")
    ...
    def _calculate_performance_metrics(self, test_results):
        ...
        detection_times = [r.get('detection_time', 0) for r in test_results if 'detection_time' in r]
        
        metrics = {
            'success_rate': sum(1 for r in test_results if r.get('detected')) / len(test_results),
            'avg_confidence': np.mean([r.get('detection_confidence', 0) for r in test_results])
        }
...
class CalibrationTools:
    ...
    def generate_calibration_pattern(self, pattern_type: str = 'checkerboard',
                                    size: Tuple[int, int] = (9, 6)) -> np.ndarray:
    ...
    def calibrate_camera(self, images: List[np.ndarray],
                        pattern_size: Tuple[int, int]) -> Dict[str, Any]:
