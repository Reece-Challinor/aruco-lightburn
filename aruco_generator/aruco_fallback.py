...
class ArUCOGenerator:
    ...
    def generate_marker(
        ...
        if marker_id >= dict_info["max_ids"]:
            raise ValueError(
                f"Marker ID {marker_id} exceeds maximum for {dict_name} ({dict_info['max_ids']})"
            )

        # Get base pattern
        if (
            dict_name in self._aruco_patterns
            and marker_id in self._aruco_patterns[dict_name]
        ):
        ...
        scaled_pattern = np.repeat(
            np.repeat(base_pattern, scale_factor, axis=0), scale_factor, axis=1
        )
        ...
        if scaled_pattern.shape[0] != size_pixels:
            ...
            for i in range(size_pixels):
                for j in range(size_pixels):
                    ...
                    src_j = int(j / x_scale)
                    if (
                        src_i < scaled_pattern.shape[0]
                        and src_j < scaled_pattern.shape[1]
                    ):
